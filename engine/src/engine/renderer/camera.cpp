...
engine::perspective_camera::perspective_camera( 
    ...
    m_far_plane(far_z) 
{ 
    m_position = glm::vec3(10.f, 40.f, 0.f);
...
{ 
    ...
    // inverting the transform matrix  
    //m_view_mat = glm::inverse(transform);
    //Changed the update_view_matrix so that it only looks at the set object when in a cutscene
    if (!in_cutscene())
    {
        m_view_mat = glm::lookAt(m_position, m_position + m_front_vector, m_up_vector);
    }
...
void engine::perspective_camera::minimap_view(glm::vec3 playerPos, glm::vec3 cam_pos)
{

    m_position = cam_pos;
    glm::lookAt(playerPos, playerPos + m_front_vector, m_up_vector);
}
