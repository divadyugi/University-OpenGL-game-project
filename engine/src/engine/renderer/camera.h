...
namespace engine
{
    ...
    public: 
        perspective_camera( 
            ...
            float fov = 45.f,
            //Increased far clipping plane of the camera from 100.f to 200.f
        ...
        void set_in_cutscene(bool in_cutscene) { m_in_cutscene = in_cutscene; }

        bool in_cutscene() const { return m_in_cutscene; }

        void set_pitch(float new_pitch) { m_pitch = new_pitch; }


        bool minimap() { return m_minimap; }
        void set_minimap(bool minimap) { m_minimap = minimap; }
        void minimap_view(glm::vec3 playerPos, glm::vec3 cam_pos);
    ...
    private: 
        ...
        //Used for the intor cutscene
        bool m_in_cutscene = true;

        //Used to check if we are in minimap view or not
